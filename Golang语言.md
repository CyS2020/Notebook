...
- main函数在main包里(`package main` 文件夹、文件名不叫main)，编译时不包含main包的源代码是不会得到可执行文件的
...
- `function`: 函数像其他值一样，拥有类型，可以被赋值给其他变量，传递给函数，从函数返回
  ...
  - 同javaScript，java方法签名=函数名，参数列表，没有返回值
