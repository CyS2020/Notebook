...
- 对于一些边界情况, 使用if语句在复杂逻辑开始之前进行单独处理, 避免所有情况全部进入复杂逻辑体
...
- 数组转集合使用Arrays.stream().collect(xxx); 集合转数组xxx.toArray(); Arrays.asList()与List.subList()返回的List均为李鬼而非李逵, subList()、subMap()、subSet()均为映射
...
- Jdk或其他框架会提供很多工具类：String、Files、Collectors、Collections、Arrays / ObjectUtils、BeanUtils、StringUtils、CollectionUtils
