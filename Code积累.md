...
- intValue()是Number类的方法， Number是一个抽象类。Java中所有的数值类都继承它。也就是说，不单是Integer有intValue方法，Double，Long等都有此方法。Number对于6种数值类型的都有对应的抽象方法；在强转过程中 浮点数转整型会有精度丢失, 表示范围大的数值转范围小的类型高位丢失
数值类型往下强转(long -> int)会丢失高位数据
...
- jvm诊断思路: 先查看线程堆栈信息, 再查看内存占用信息, 最后看看启动参数
